//input restricted double ended circular queue
#include<stdio.h>
#include<stdlib.h>
...
int remove_rear(int q[],int n, int*front, int*rear);
...
int main()
{
    ...
    while(1)
    {
        ...
        switch(c)
        {
            ...
            {
                int value;
                value=remove_rear(queue,n,&front,&rear);
                if(value!=-1)
                {
                    printf("\nValue %d removed from rear",value);
                }
                break;
            }
            case 4:
